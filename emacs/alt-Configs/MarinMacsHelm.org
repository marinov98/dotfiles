...
- Helm-descbinds: Helm interface for describe bindings command
  #+begin_src emacs-lisp
    (use-package helm
        :ensure t
        :diminish
     ...
     (use-package helm-ag
         :after helm
         :custom
         (helm-ag-base-command "rg --vimgrep --no-heading --smart-case") ;; force use of ripgrep, delete this customization to go back to ag
         :ensure t)
...
#+BEGIN_SRC emacs-lisp
  (use-package smerge-mode
    ...
    :hook (magit-diff-visit-file . (lambda ()
                                     (when smerge-mode
                                       (hydra-smerge/body))))) ;; launch hydra-smerge when a merge conflict occurs
#+END_SRC
* Hydra
 :PROPERTIES:
 :CUSTOM_ID: Hydra
...
#+BEGIN_SRC emacs-lisp
    (pretty-hydra-define hydra-projectile (:color red :title "🚀 Projectile 🚀" :quit-key "q") 
     ("Finding"
      ...
      "Text"
      (("a" helm-projectile-ag "ag") ;; I forced ag to be rg so this currently does not work use SPC / instead
      ("g" helm-projectile-rg "rg")) ;; this also does not work because helm-rg is currently having issues at the time of me making this
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-window (:color pink :title "⚡⚡ Helm + Windows ⚡⚡" :quit-key "q") 
   ("Helm"
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-lsp (:color red :title "📡 LSP 📡" :quit-key "q") 
    ...
    "Jump"
    (("b" switch-to-prev-buffer "back") ;; better consistency than lsp-ui-peek-jump-backward
    ("n" switch-to-next-buffer "forward")) ;; better consistency than lsp-ui-peek-jump-forward
