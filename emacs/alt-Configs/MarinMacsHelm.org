...
#+BEGIN_SRC emacs-lisp
  (setq default-frame-alist '((font . "Fira Code-12"))) ;; set font, font size, and cursor color
...
- Swiper: an alternative to emacs’s native isearch that can ivy or helm to show an overview of matches
- Helm-rg: uses ripgrep with helm frontend to find matches
- Helm-ag: uses silversearcher with helm frontend to find matches
- Helm-fzf: uses fuzzy file finding with helm frontend
- Helm-ls-git: allows you search following your gitignore
- Helm-swoop: listing line matches 
  #+begin_src emacs-lisp
    (use-package helm
        :ensure t
        :diminish
  ...
  #+end_src
Helm did not seem to provide fzf functionality and helm-fzf does not seem to exist anymore unlike counsel, so this was a workaround
#+begin_src emacs-lisp
  (use-package fzf
      :ensure t)
    ;; :init
