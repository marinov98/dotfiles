...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package neotree
      ...
      (setq neo-theme (if (display-graphic-p) 'icons 'arrow))) ; add icons
#+END_SRC
** Iedit
#+BEGIN_SRC emacs-lisp
(use-package iedit
...
(setq-default flycheck-disabled-checkers '(c/c++-clang c/c++-cppcheck c/c++-gcc)) ;; disable other checkers since we are using clangd
