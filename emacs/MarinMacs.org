...
* General Settings 
 ...
 :END:
Garbage collection and Lexical binding
#+BEGIN_SRC emacs-lisp
    ;; -*- lexical-binding: t; -*-

    ;; disable garbage collection on start up 
    (eval-when-compile
...
- *Depreciated* lsp and multiple-cursors do the job best
#+BEGIN_SRC emacs-lisp
(use-package iedit
    :disabled
    :ensure t
...
#+BEGIN_SRC emacs-lisp
     ...
     (defhydra hydra-window (:color pink :columns 3)
      "⚡⚡ Ivy + Windows ⚡⚡"
...
*** Launcher (launch stuff)
 ...
 ("." . browse-url-chromium)))
  ...
  (defhydra hydra-launcher (global-map "C-c l" :color red :columns 2)
   ...
   ("G" (browse-url "https://github.com/marinov98") "my GitHub")
   ("r" (browse-url "https://www.netflix.com/") "netflix")
   ("y" (browse-url "https://www.youtube.com/") "YouTube")
   ("s" better-shell-shell "shell")
   ("q" nil "quit"))
  #+END_SRC
* LSP
 :PROPERTIES:
...
* Development
 ...
 :END:
 These bindings have been quite useful
 #+begin_src  emacs-lisp
