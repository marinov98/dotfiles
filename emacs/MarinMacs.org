...
#+END_SRC
- options, display, parenthesis, time-mode, line numbers
- *IMPORTANT* I am experimenting with line numbers
  - I say this because I am not sure if all packages work with them properly, but so far everything works
...
#+BEGIN_SRC emacs-lisp
    (setq default-frame-alist '((font . "Fira Code-10")))  ;; set font and font size
...
- If you are a *react.js* developer, I *highly recommend* hooking *rjsx-mode* to *.js* files as well
  - if you are a *node.js* developer and/or *vanilla js* developer, the current setup with web mode is better for syntax highlighting
