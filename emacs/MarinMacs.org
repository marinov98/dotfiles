...
#+END_SRC
*** Elpy
 - I typically prefer LSP but the python server has been unstable so this is an alternative
...
#+end_src
*** Company-jedi
 Lightweight auto-completion alternative to =lsp= (python server seems to be slow) and =elpy= (a bit bloated in my opinion)
- need to install =virtualenv= via pip for when you do =M-x jedi:install-server= to enable the completion
    #+begin_src emacs-lisp
      (use-package company-jedi
          :ensure t
          :config
          (defun marinov/add-completion ()
            (add-to-list 'company-backends 'company-jedi)
            (define-key python-mode-map (kbd "C-c d") 'jedi:goto-definition)
            (define-key python-mode-map (kbd "C-c s") 'jedi:show-doc))
	    
          (add-hook 'python-mode-hook 'jedi:setup)
          (add-hook 'python-mode-hook 'marinov/add-completion))
    #+end_src
*** Virtualenv
