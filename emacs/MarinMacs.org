...
* Modeline
 ...
 :CUSTOM_ID: modeline
 :END:
- Modelines are at the bottom of the window, they  describes what is going on in the current buffer
  - they display modes, times,filenames, and even line numbers
...
* Globals
 ...
 :CUSTOM_ID: globals
 :END:
 The below packages provide minor modes that will be enabled every buffer (hence the global)
** Undo-Tree
 :PROPERTIES:
 :CUSTOM_ID: undo-tree
 :END:
 ...
 - Even if you are using emacs without this evil-mode, this package can prove useful
...
- By itself it does not do much, you need to add backends to it for your specific development language to get autocomplete
  - In my case I add the company-lsp(seen further in the config) backend which allows for language servers to take care of my auto-complete
