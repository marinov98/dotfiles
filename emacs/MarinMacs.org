...
#+BEGIN_SRC emacs-lisp
    ...
    (when (and (version<= "29.1" emacs-version) (not (string-equal system-type "windows-nt")))
...
#+BEGIN_SRC emacs-lisp
      ...
      (use-package evil-mc
          ...
          (define-key evil-normal-state-map (kbd "gc") 'hydra-mc/body))
...
#+BEGIN_SRC emacs-lisp
  (use-package dashboard
      ...
      :init
      (if (file-directory-p "~/.emacs.d/img")
            (setq dashboard-startup-banner "~/.emacs.d/img/targetBanner.png")
...
#+BEGIN_SRC emacs-lisp
  (use-package projectile
      ...
      :init
      (when (file-directory-p "~/Projects/git") ;; change this to whatever directory your projects are in
...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package magit-repos
        ...
        :config
        (when (file-directory-p "~/Projects/git") ;; change this to whatever directory your projects are in
          (setq magit-repository-directories '(("~/Projects/git" . 1)))))
...
#+BEGIN_SRC emacs-lisp  
     (use-package lsp-mode
         ...
         (lsp-signature-auto-activate nil) ;; you could manually request them via `lsp-signature-activate (This tends to cause problems and is slow ESP on MS Windows)
         ...
         :config
         (add-hook 'after-init-hook #'(lambda() ;; in case I disable lsp-ui remap g prefix keys to regular lsp
