...
#+BEGIN_SRC emacs-lisp
     (when (version<= "26.0.50" emacs-version )         
       ...
       (global-visual-line-mode t)) ;; display line numbers in every buffer
...
#+BEGIN_SRC emacs-lisp
  (use-package general
    ...
    (general-create-definer mpm/leader-keys
       ...
       :global-prefix "C-S-SPC") ;; access leader in insert mode

    (mpm/leader-keys
       "SPC" '(hydra-projectile/body :wk "Projectile Hydra")
       "w" '(hydra-window-mini/body :wk "Window Hydra")
       "S" '(hydra-smerge/body :wk "Smerge Hydra")
       "l" '(hydra-lsp/body :wk "LSP Hydra")
       "d" '(hydra-describe/body :wk "Describe Hydra")
       "g" '(hydra-git/body :wk "Git Hydra")
       "TAB" '(hydra-launcher/body :wk "Launcher Hydra")
...
** Ranger
 ...
 :CUSTOM_ID: ranger
 :END:
 - An alternative to dired. (Probably will remove soon but use if you would like)
...
** Undo-Tree
 ...
 :CUSTOM_ID: undo-tree
 :END:
 - You MUST have one of the following for [[#evil-mode][Evil Mode]] to work
...
#+BEGIN_SRC emacs-lisp
    (use-package company
        ...
        (:map company-active-map
            ...
            ("M-j" . company-select-next)  ;; use M-j and M-k to navigate completions (can use C-n and C-p as well I believe)
            ("M-k"  . company-select-previous))
        (:map lsp-mode-map
            ("<tab>" . company-indent-or-complete-common))
        :custom
        (company-tooltip-limit 5) ; show 5 candidates at one time
        (company-idle-delay 0.0) ;; delay (in seconds) when candidates are shown, change if you need to, potentially cpu intensive on older machines if set to 0
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-projectile (:color red :title "ðŸš€ Projectile ðŸš€" :quit-key "q") 
    ("Finding"
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-window-mini (:color blue :title "âš¡ Window Nav âš¡" :quit-key "q") 
    ("Split"
    (("2" split-window-right "v-split")
    ("3" split-window-below "h-split"))

    "Full Window Hydra"
    (("w" hydra-window/body  "Full Window Hydra"))
  ...
  (pretty-hydra-define hydra-window (:color pink :title "âš¡âš¡ Ivy + Windows âš¡âš¡" :quit-key "q") 
    ("Ivy"
...
*** Smerge (for handling merge conflicts)
    #+begin_src emacs-lisp
   (pretty-hydra-define hydra-smerge (:color pink :title "âš¡ Smerge âš¡" :quit-key "q")
     ("Move"
     (("n" smerge-next)
     ("p" smerge-prev))
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-lsp (:color red :title "ðŸ“¡ LSP ðŸ“¡" :quit-key "q") 
    ("Find"
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-avy (:color blue :title "â†µ Avy â†µ" :quit-key "q")
    ("Char"
...
#+BEGIN_SRC emacs-lisp
  (pretty-hydra-define hydra-utility (:color red :title "ðŸ˜Ž Utility ðŸ˜Ž" :quit-key "q")
    ("Search"
...
#+BEGIN_SRC emacs-lisp  
    (use-package lsp-mode
        ...
        :diminish
        :hook
        ((c++-mode c-mode python-mode css-mode yaml-mode json-mode js-mode js2-mode rjsx-mode java-mode typescript-mode web-mode) . lsp-deferred)
        :custom
        (lsp-idle-delay 0.1)
        :config
    ...
    (use-package lsp-ui
        ...
        :custom
        (lsp-ui-doc-enable t)
        (lsp-ui-doc-delay 0.4) ;; display doc after 4/10 of a second
        ...
        :config
        (setq eldoc-idle-delay 0.5)) ;; delay eldoc for 5/10 second
